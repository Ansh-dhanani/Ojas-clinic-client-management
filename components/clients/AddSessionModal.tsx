...
export function AddSessionModal({ clientId, sessionNumber, onClose }: AddSessionModalProps) {
  ...
  const [sessionsOnDate, setSessionsOnDate] = useState<number>(0)

  const checkSessionsOnDate = async (dateStr: string) => {
    if (!dateStr) {
      setSessionsOnDate(0)
      return
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await createSession.mutateAsync({
      clientId,
      sessionNumber: `S${sessionNumber.toString().padStart(3, '0')}`,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full p-6 my-8 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6 sticky top-0 bg-white pb-4 border-b">
          <h3 className="text-2xl font-bold text-gray-900">Add Session #{sessionNumber}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  setFormData({ ...formData, nextSessionDate: e.target.value })
              ...
              {formData.nextSessionDate && sessionsOnDate > 0 && new Date(formData.nextSessionDate) > new Date() && (
                <p className="mt-1 text-sm text-orange-600 font-medium">
                  ⚠️ {sessionsOnDate} session{sessionsOnDate > 1 ? 's' : ''} already scheduled on this date
                </p>
              )}
            </div>
          ...
          </div>          {/* Before Condition Assessment */}
              ...
              <div>
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
                  />
                  <span className="text-sm font-medium text-gray-700">Mark Treatment as Completed</span>
                </label>
              </div>
            </div>
          </div>
